### python frame_wise_analysis.py --object_detector ./models/model_name(with_extension) --input_path 'inputs/file_name(with_extention)' --frames_per_second 20 --output_path 'outputs/file_name(no_extension_needed)'
...
# Color indexing for the PI plot
color_index = {

                   'bus': 'red', 'handbag': 'steelblue', 'giraffe': 'orange', 'spoon': 'gray', 
                   'cup': 'yellow', 'chair': 'green', 'elephant': 'pink', 'truck': 'indigo', 'motorcycle': 'azure',
                   ...
                   'bench': 'salmon', 'bottle': 'brown', 'car': 'silver', 'bowl': 'maroon', 'tennis racket': 'palevilotered',
                   ...
                   'traffic light': 'mediumblue', 'bed': 'navy', 'broccoli': 'royalblue', 'backpack': 'slategray', 
                   'snowboard': 'skyblue', 'kite': 'cadetblue', 'teddy bear': 'peacock', 'clock': 'lightcyan', 
                   'wine glass': 'teal', 'frisbee': 'aquamarine', 'donut': 'mincream', 'suitcase': 'seagreen', 
                   'dog': 'springgreen', 'banana': 'emeraldgreen', 'person': 'honeydew', 'surfboard': 'palegreen', 
...
# Analysis of each frame with visualization.
...
video_detector.setModelPath(args["object_detector"])
video_detector.loadModel()
plt.show()

video_detector.detectObjectsFromVideo(input_file_path=args["input_path"],
                            output_file_path=args["output_path"], frames_per_second= int(args["frames_per_second"]), 
                            per_frame_function=forFrame, minimum_percentage_probability=30, return_detected_frame=True)
